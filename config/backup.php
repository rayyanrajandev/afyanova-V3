...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * The application code is already in version control — only
                 * back up what isn't: uploaded/generated files under
                 * storage/app (patient documents, generated reports, etc).
                 */
                'include' => [
                    storage_path('app'),
                ],
             ...
             */
            'databases' => [
                // On Postgres, dump via the dedicated BYPASSRLS backup role
                // (see config/database.php's 'pgsql_backup' connection) —
                // the app's own role is deliberately blocked from reading
                // across tenants by FORCE ROW LEVEL SECURITY, which would
                // otherwise break a full pg_dump too.
                env('DB_CONNECTION') === 'pgsql' ? 'pgsql_backup' : env('DB_CONNECTION', 'sqlite'),
            ],
        ],
